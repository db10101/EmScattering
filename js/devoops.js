...
function drawTheGoogleChart(chart_data, chart_options, element, chart_type) {
	// Function for visualize Google Chart
	// var data = google.visualization.arrayToDataTable(chart_data);
	var chart = new chart_type(document.getElementById(element));
...
function DrawAllCharts(){
    ...
    var layers = parseInt(document.getElementById("ui-spinner").value)+2;
     ...
     var transData = new google.visualization.DataTable();
      transData.addColumn('number', 'omega');
      for(var i = 0; i < transmissionGraph.kzList.length; i++){
        transData.addColumn('number', 'kz'+transmissionGraph.kzList[i]);
      }
	...
	var chart2_type = google.visualization.LineChart;
	drawTheGoogleChart(chart2_data, chart2_options, chart2_element, chart2_type);
	drawTheGoogleChart(chart2_data, chart2_options, "google-chart-6", chart2_type);


	//  Chart 3
	var dispersion = crystal.dispersionRelationship(k1,k2,range,100);
	 var dispData = new google.visualization.DataTable();
	  dispData.addColumn('number', 'kz');
	  for(var i = 0; i < dispersion.layersDispersions.length; i++){
	    dispData.addColumn('number', 'Layer'+i);
	  }
	...
	var chart3_type = google.visualization.LineChart;
	drawTheGoogleChart(chart3_data, chart3_options, chart3_element, chart3_type);
	drawTheGoogleChart(chart3_data, chart3_options, "google-chart-5", chart3_type);
	//  Chart 4
	// var chart4_data = [
    ...
    //  }
          for (var i = 0; i < interfaces.length-1; i++) {
               ...
               document.querySelector('.overlay'+i).style.left = Math.floor(cli.getXLocation(0)) + 20 + "px";
          };
	...
	google.visualization.events.addListener(chart6_type, 'ready', printInterfaces.bind(chart6_type, chart6_data));
	drawGoogleChart(chart6_data, chart6_options, chart6_element, chart6_type);
	drawGoogleChart(chart6_data, chart6_options, "google-chart-4", chart6_type);
